...
// =============================================
// SCROLL REVEAL (masonry, staggered)
// =============================================
...
filterBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        items.forEach(item => {
            ...
            item.style.transform = show ? 'translateY(0)' : 'translateY(8px)';
        });
    });
});
...
function openModal(src) {
    ...
    modalImg.src = src;
    currentImageIndex = galleryImageSources.findIndex(s => s.endsWith(src.replace('Images/', '')));
...
// Masonry items now use data-img instead of inline onclick="" so this
// keeps working under a strict CSP (script-src 'self', no 'unsafe-inline').
document.querySelectorAll('.masonry-item[data-img]').forEach(item => {
